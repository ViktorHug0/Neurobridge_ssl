...
\begin{frame}{Route 1 --- Render and caption}
  \small
  We reconstruct the picture first, then describe it with an ordinary image captioner.
  This is the direct pipeline, with no training required on our part.
  ...
  \begin{itemize}
    \item \textbf{Encoder.} ENIGMA, the current state of the art for EEG-to-image on this dataset.
          It outputs a 1024-d CLIP-aligned embedding for each EEG sample.
    \item \textbf{Generator.} The predicted embedding enters SDXL-Turbo with IP-Adapter to replace the real image embedding.
          We use four denoising steps and no additional guidance. Output is $256\times256$.
...
\begin{frame}{The Q-Former}
  ...
  \vspace{0.7em}
  We replace the Q-Former with a \textbf{bridge}: a single linear layer
...
\begin{frame}{Bridge 1 --- Error-matched bridge}
  \small
  Key idea: We skip the picture. Instead, we learn to map the predicted embedding straight onto BLIP-2's query tokens.
  ...
  \vspace{0.55em}
  \begin{itemize}
    \item \textbf{Calibrating the bridge on ENIGMA's own predictions.} The bridge is fitted on ENIGMA's \emph{own
          predictions} for the 16{,}540 training images, not on real image embeddings. It therefore
          sees the distribution of the encoder's errors rather than clean image vectors.
    \item \textbf{Correcting for optimism.} The cosine between ENIGMA's predicted embedding and the true embedding is $0.531$ on
          ...
          $0.531/\sqrt{1+1.08^{2}}=0.360$, so the bridge trains at the error level it will meet at test.
  \end{itemize}

  \takeaway{Train the bridge on the distribution it will actually be given, rather than on
  clean data it will never see again.}
\end{frame}

\begin{frame}{Bridge 2 --- Reconstruction-tuned bridge}
  \small
  Same bridge idea, but our own encoder (no ENIGMA anymore) and our own embedding space, and the noise is simulated
  ...
  \vspace{0.55em}
  \begin{itemize}
    \item \textbf{Encoder.} We use our best encoder (in term of retrieval performance), aligned to InternViT-6B layer 28 rather than a final CLIP layer.
          The loss is $0.3\cdot\mathrm{InfoNCE}+0.7\cdot\mathrm{MSE}$ on unnormalised features:
          The additional MSE term is the one that helps with reconstruction. It was swept over twelve variants.
...
\begin{frame}{Metrics choice}
  \small
  Metric choice was particularly sensitive and noisy. Still unsure which to chose.

  \vspace{0.45em}
  \begin{itemize}
    \item \textbf{Identification.} Encode the caption with CLIP's text encoder and retrieve the
          true image among the 200 test images. Chance is $0.5\pct$.
...
\begin{frame}{Results}
  ...
  \vspace{0.5em}
  \small The ceiling is the same bridge fed a real image embedding instead of an EEG one, i.e. a bridge that would be given the true image embedding at test.
  We can see it comes very close to the reference, so the bridge is not the limiting factor. 
...
\begin{frame}{Metrics disagreement}
  \small
  \begin{columns}[T,onlytextwidth]
