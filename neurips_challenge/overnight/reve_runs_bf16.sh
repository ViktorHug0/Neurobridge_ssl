#!/bin/bash
# REVE fine-tuning plan (2026-09-23), bf16 autocast. usage: reve_runs_bf16.sh r1|r123
...
case "$1" in
  ...
  r1)   step env OUT_DIR=./results/things_eeg/neurips_track1/reve/r1_in120_bf16 \
          bash neurips_challenge/run_reve_repro.sh $IN120 --amp_dtype bfloat16 ;;
  # R1+R2+R3: + head lr 1e-3 / backbone 1e-4, 1 head-only epoch, REVE flatten head.
  r123) step env OUT_DIR=./results/things_eeg/neurips_track1/reve/r123_in120_lr_head_bf16 \
          bash neurips_challenge/run_reve_repro.sh $IN120 --amp_dtype bfloat16 --learning_rate 1e-3 \
  ...
  r12)  step env OUT_DIR=./results/things_eeg/neurips_track1/reve/r12_in120_lr_bf16 \
          bash neurips_challenge/run_reve_repro.sh $IN120 --amp_dtype bfloat16 --learning_rate 1e-3 \
