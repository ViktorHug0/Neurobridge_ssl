...
readonly ENCODER=TinyGraphDiffusionNet
readonly MEMBER_NAME=tiny_graph_diffusion_seed7200
readonly RESULT_ROOT=${RESULT_ROOT:-$REPO_ROOT/results/things_eeg/tiny_graph_diffusion/testselected_internvit28}
...
if [ -f "$dump" ]; then
  echo "[tiny-graph] existing export found; skipping target=$TARGET"
  exit 0
fi
