...
case "$ARM" in
  A)
    readonly ENCODER=TinyGatedChannelTransformer
    readonly MODEL_SEED=7200
    readonly MEMBER_NAME=tiny_gated_channel_seed7200
    readonly RESULT_ROOT=${RESULT_ROOT:-$REPO_ROOT/results/things_eeg/tiny_gated_attention/arm_a_channel}
    ;;
  DIFF)
    readonly ENCODER=TinyDifferentialChannelTransformer
    readonly MODEL_SEED=7200
    readonly MEMBER_NAME=tiny_differential_channel_seed7200
    readonly RESULT_ROOT=${RESULT_ROOT:-$REPO_ROOT/results/things_eeg/tiny_gated_attention/arm_e_differential}
    ;;
  *)
    echo "ARM must be A or DIFF, got: $ARM" >&2
    exit 2
    ;;
esac
...
if [ -f "$dump" ]; then
  echo "[tiny-attention] existing export found; skipping arm=$ARM target=$TARGET"
  exit 0
fi
...
mapfile -t run_dirs < <(find_completed_runs)
if [ "${#run_dirs[@]}" -eq 0 ]; then
  echo "no completed $ENCODER run found" >&2
  exit 2
fi
