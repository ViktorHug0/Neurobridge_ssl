#!/bin/bash
# group_e100 recipe probe: 9-source Dirichlet SubjectMix + 100 epochs, the recipe
# behind the repo best single model (37.00). The ortho arms overfit at epoch ~25,
# so stronger cross-subject mixing is the indicated lever.
#
...
OUT=$PWD/results/things_eeg/ortho_arch/${ARM}_group/seed3300
mkdir -p "$OUT"
...
for held in ${FOLDS:-$(seq 1 10)}; do
  ...
  python train.py \
    --batch_size 1024 --num_workers ${NW:-8} --learning_rate 3e-4 --num_epochs 100 \
    ...
    --mixup_type group --subject_mixup_alpha 0.5 --subject_mixup_mode raw_eeg --subject_mixup_prob 1.0 \
