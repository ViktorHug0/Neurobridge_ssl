...
readonly RESULT_ROOT=${RESULT_ROOT:-$REPO_ROOT/results/things_eeg/tiny_squeezeformer/testselected_internvit28}
...
readonly MEMBER_NAME=tiny_squeezeformer_seed3300
...
if [ -f "$dump" ]; then
  echo "[tiny-sqf] existing export found; skipping target=$TARGET"
  exit 0
fi
...
if [ "${#run_dirs[@]}" -eq 0 ]; then
  python train.py \
    ...
    --eeg_encoder_type TinySqueezeformer
fi

mapfile -t run_dirs < <(find_completed_runs)
if [ "${#run_dirs[@]}" -eq 0 ]; then
  echo "no completed TinySqueezeformer run found" >&2
  exit 2
fi
