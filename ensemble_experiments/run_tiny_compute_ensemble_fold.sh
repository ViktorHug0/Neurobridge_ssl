...
RESULT_ROOT=${RESULT_ROOT:-$REPO_ROOT/results/things_eeg/tiny_compute_ensemble/testselected_internvit28}
...
member_name="${family}_seed${model_seed}"
...
aggregate() {
  flock "$RESULT_ROOT/.aggregate.lock" \
    python -m ensemble_experiments.analyze_tiny_compute_ensemble \
      --result-root "$RESULT_ROOT"
}

if [ -f "$dump" ]; then
  echo "[tiny-ensemble] existing export found; skipping member=$MEMBER target=$TARGET"
  aggregate
  exit 0
fi
...
echo "[tiny-ensemble] member=$MEMBER name=$member_name target=$TARGET"
if [ "${#run_dirs[@]}" -eq 0 ]; then
  ...
  echo "[tiny-ensemble] completed checkpoint found; skipping training"
fi

mapfile -t run_dirs < <(find_completed_runs)
if [ "${#run_dirs[@]}" -eq 0 ]; then
  echo "expected at least one completed model directory, found none" >&2
  exit 2
fi
