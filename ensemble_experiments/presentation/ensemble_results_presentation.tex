...
\title{Monthly meeting\\27/08/2026}
...
\begin{frame}{A score-level error model}
  ...
  \vspace{0.35em}
  \textcolor{nbgray}{This is a working decomposition, not an identifiable generative model:
  the boundary between $\eta$ and $\xi_i$ depends on the model population.}

  \takeaway{Members should agree on $\mu$; averaging can attenuate $\xi_i$, but not shared error $\eta$.}
\end{frame}

\begin{frame}{What score averaging removes}
  \small Uniform averaging preserves the common terms and averages the residuals:
  \[
    \bar z=\mu+\eta+\bar\xi,
    \qquad \bar\xi=\frac{1}{K}\sum_{i=1}^{K}\xi_i.
  \]
  If $\operatorname{Var}(\xi_i)=\sigma_\xi^2$ and residual pairs share nonnegative correlation $\rho_e$,
  \[
    \operatorname{Var}(\bar\xi)
    =\sigma_\xi^2\!\left[\frac{1}{K}+\left(1-\frac{1}{K}\right)\rho_e\right]
    =\sigma_\xi^2\!\left[\rho_e+\frac{1-\rho_e}{K}\right].
  \]

  \begin{columns}[T,onlytextwidth]
    \column{0.48\textwidth}
      \begin{itemize}
        \item lower residual covariance $\Rightarrow$ more removable error
        \item $K\!\to\!\infty$: floor $\rho_e\sigma_\xi^2$
      \end{itemize}
    \column{0.48\textwidth}
      \begin{itemize}
        \item largest benefit is expected from early members
        \item one deterministic model has no across-function average; stochastic model averaging can approximate one
      \end{itemize}
  \end{columns}

  \textcolor{nbgray}{Measured score correlation is not $\rho_e$: shared components $\mu+\eta$ also raise it.}
\end{frame}

\begin{frame}{Why retrieval amplifies variance reduction}
  \small Top-1 is a threshold event against the strongest of 199 distractors:
  \[
    \operatorname{Acc}
    =\Pr\!\left[\bar z(t,c^*)>\max_{c\neq c^*}\bar z(t,c)\right].
  \]
  ...
  \begin{tabular}{@{}p{0.43\textwidth}p{0.47\textwidth}@{}}
    \toprule
    Mechanism & Observed signature \\
    \midrule
    many trials near the top-rank boundary & modest score stabilization flips many argmax decisions \\
    heterogeneous biases lower residual covariance & ATM + TSConv gives the first large gain \\
    $1/K$ term decays quickly & gains are front-loaded at $k=2$ \\
    shared error and covariance remain & committees saturate after a few members \\
    \bottomrule
  \end{tabular}
  \end{center}

  \takeaway{This explains the observed pattern, but is not a universal guarantee: weak or similarly biased members can hurt.}
\end{frame}
...
\begin{frame}{References: EEG decoding}
  \small
...
\begin{frame}{References: ensemble theory}
  ...
  \vspace{0.8em}
  \textcolor{nbgray}{Classical variance--covariance analysis motivates the score model;
  modern deep-ensemble evidence emphasizes diversity in function space.}
\end{frame}
