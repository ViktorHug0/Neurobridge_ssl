...
: "${MEMBER:?MEMBER is required (0=UltraTinyTSConv, 1=UltraTinyATM)}"

REPO_ROOT=/nasbrain/p20fores/Neurobridge_SSL
RESULT_ROOT=${RESULT_ROOT:-$REPO_ROOT/results/things_eeg/ultratiny_reference_pair/testselected_internvit28}
...
case "$MEMBER" in
  0)
    member_name=ultratiny_tsconv_seed5300
    model_seed=5300
    model_args=(
      --eeg_encoder_type TSConv_parameterizable
      --eeg_backbone_dim 128
      --tsconv_temporal_filters 6
      --tsconv_temporal_kernel 25
      --tsconv_pool_kernel 51
      --tsconv_pool_stride 5
      --tsconv_spatial_filters 6
      --tsconv_projection_filters 6
    )
    ;;
  1)
    member_name=ultratiny_atm_seed6300
    model_seed=6300
    model_args=(
      ...
      --atm_e_layers 1
      --atm_d_ff 80
      --atm_temporal_filters 8
      --atm_temporal_kernel 15
      --atm_pool_kernel 31
      --atm_pool_stride 5
      --atm_spatial_filters 8
      --atm_projection_filters 8
    )
    ;;
  *)
...
if [ -f "$dump" ]; then
  echo "[ultratiny] existing export found; skipping member=$MEMBER target=$TARGET"
  exit 0
fi
...
mapfile -t run_dirs < <(find_completed_runs)
if [ "${#run_dirs[@]}" -eq 0 ]; then
  echo "no completed UltraTiny run found" >&2
  exit 2
fi
