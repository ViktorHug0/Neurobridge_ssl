...
CONFIG_NAME="tsconv_fd${FEATURE_DIM}_mixup_raw_pairwise_linear_a${MIXUP_ALPHA//./p}"
EXTRA_ARGS="--eeg_encoder_type TSConv --projector linear --feature_dim ${FEATURE_DIM} --eeg_backbone_dim ${EEG_BACKBONE_DIM} --subject_mixup_mode raw_eeg --mixup_type pairwise --subject_mixup_alpha ${MIXUP_ALPHA} --multi_positive_loss --grouped_batch_sampler --samples_per_image ${SAMPLES_PER_IMAGE}"

RUN_TAG="${RUN_TAG:-things_meg_mixup_$(date +'%Y%m%d-%H%M%S')}"
...
echo "----------------------------------------------------------"
echo "THINGS-MEG inter-subject mixup"
...
echo "THINGS-MEG mixup completed: ${UNIFIED_CSV}"
