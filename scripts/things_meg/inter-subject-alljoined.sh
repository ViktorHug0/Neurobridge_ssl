...
IMAGE_FEATURE_BASE_DIR="${IMAGE_FEATURE_BASE_DIR:-${REPO_ROOT}/data/things_meg/image_feature}"
IMAGE_ENCODER_TYPE="${IMAGE_ENCODER_TYPE:-InternViT-6B_layer28_mean_8bit_meg}"
IMAGE_FEATURE_DIR="${IMAGE_FEATURE_DIR:-${IMAGE_FEATURE_BASE_DIR}/${IMAGE_ENCODER_TYPE}}"
TEXT_FEATURE_DIR="${TEXT_FEATURE_DIR:-}"
EEG_DATA_DIR="${EEG_DATA_DIR:-${REPO_ROOT}/data/things_meg/preprocessed_meg}"
...
SEEDS="${SEEDS:-3300}"
SUBJECT_IDS="${SUBJECT_IDS:-1 2 3 4}"
...
RUN_TAG="${RUN_TAG:-things_meg_compare_$(date +'%Y%m%d-%H%M%S')}"
...
CONFIG_NAMES=(
    "tsconv_fd${FEATURE_DIM}_nomix_linear"
    "tsconv_fd${FEATURE_DIM}_mixup_raw_pairwise_linear_a${MIXUP_ALPHA//./p}"
)
CONFIG_ARGS=(
    "--eeg_encoder_type TSConv --projector linear --feature_dim ${FEATURE_DIM} --eeg_backbone_dim ${EEG_BACKBONE_DIM} --subject_mixup_mode none --multi_positive_loss --grouped_batch_sampler --samples_per_image ${SAMPLES_PER_IMAGE}"
    "--eeg_encoder_type TSConv --projector linear --feature_dim ${FEATURE_DIM} --eeg_backbone_dim ${EEG_BACKBONE_DIM} --subject_mixup_mode raw_eeg --mixup_type pairwise --subject_mixup_alpha ${MIXUP_ALPHA} --multi_positive_loss --grouped_batch_sampler --samples_per_image ${SAMPLES_PER_IMAGE}"
)

echo "----------------------------------------------------------"
echo "THINGS-MEG inter-subject comparison"
...
do
    ...
    do
        ...
        echo "=========================================================="
        echo "Running THINGS-MEG comparison: ${CONFIG_NAME} (Seed ${SEED})"
...
echo "THINGS-MEG comparison completed: ${UNIFIED_CSV}"
