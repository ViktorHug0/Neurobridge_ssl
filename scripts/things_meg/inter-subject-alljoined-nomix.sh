...
CONFIG_NAME="tsconv_fd${FEATURE_DIM}_nomix_linear"
EXTRA_ARGS="--eeg_encoder_type TSConv --projector linear --feature_dim ${FEATURE_DIM} --eeg_backbone_dim ${EEG_BACKBONE_DIM} --subject_mixup_mode none --multi_positive_loss --grouped_batch_sampler --samples_per_image ${SAMPLES_PER_IMAGE}"

RUN_TAG="${RUN_TAG:-things_meg_nomix_$(date +'%Y%m%d-%H%M%S')}"
...
echo "----------------------------------------------------------"
echo "THINGS-MEG inter-subject nomix"
...
echo "THINGS-MEG nomix completed: ${UNIFIED_CSV}"
