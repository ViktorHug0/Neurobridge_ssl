#!/bin/bash
# Projector-only sweep: vary alignment dim (--feature_dim), fixed EEG backbone width (--eeg_backbone_dim).
# Same recipe as projector_only_sweep_20260423-112642 (bb=128), but backbone embedding dim = 1024.
set -e
...
FEATURE_DIM_VALUES="${FEATURE_DIM_VALUES:-16 32 64 128 256 512 1024}"
...
SESSION_DIR="${OUTPUT_DIR_BASE}/projector_only_bb256_sweep_20260520-142420" # projector_only_bb${EEG_BACKBONE_DIM_FIXED}_sweep_${SESSION_TIMESTAMP}"
SESSION_SUMMARY="${SESSION_DIR}/sweep_summary.csv"
mkdir -p "$SESSION_DIR"

BASE_EXTRA_ARGS="--multi_positive_loss --grouped_batch_sampler --samples_per_image 9 --eeg_backbone_dim ${EEG_BACKBONE_DIM_FIXED}"

echo "=============================================================="
echo "Projector-only sweep | EEG backbone dim fixed at ${EEG_BACKBONE_DIM_FIXED}"
...
do
    ...
    do
        ...
        echo "Seed: $SEED, projector_dim: $DIM, backbone_dim: ${EEG_BACKBONE_DIM_FIXED}"
        echo "##########################################################"
...
echo "Projector-only (bb=${EEG_BACKBONE_DIM_FIXED}) sweep completed: $SESSION_SUMMARY"
