...
DROP_SESSION="${DROP_SESSION:-alljoined_compare_XXXXX}"
OUTPUT_ROOT="${OUTPUT_DIR:-${REPO_ROOT}/results/alljoined/inter-subjects}"
SOURCE_RUN_ROOT="${SOURCE_RUN_ROOT:-${OUTPUT_ROOT}/${DROP_SESSION}}"
SEEDS=(3300)
...
K_VALUES="${K_VALUES:-3}"
SAW_VALUES="${SAW_VALUES:-0.85 0.90 0.95}"
SOFT_STEP_VALUES="${SOFT_STEP_VALUES:-10 12 14}"
SOFT_POWER_VALUES="${SOFT_POWER_VALUES:-1.0}"
SINKHORN_ITER_VALUES="${SINKHORN_ITER_VALUES:-8 10 12}"
...
if [ "$SWEEP_DIM" = "all" ]; then
    for d in k saw steps power iters; do
        run_one_dimension "$d"
    done
