#!/bin/bash
# Sparse alignment inter-subject LOSO: bb128 + feature_dim 8192 + ReLU + subject mixup.
# Trains one model per held-out subject, then records/plots per-epoch accuracy & sparsity
# and train-set active-feature coverage at the best checkpoint.
set -e
...
SESSION_DIR="${SESSION_DIR:-${OUTPUT_DIR_BASE}/sparse_bb${EEG_BACKBONE_DIM_FIXED}_fd${FEATURE_DIM}_mixup_seed${SEED}_${SESSION_TIMESTAMP}}"
CONFIG_NAME="sparse_relu_fd${FEATURE_DIM}_bb${EEG_BACKBONE_DIM_FIXED}_mixup_seed${SEED}"
...
BASE_EXTRA_ARGS="--multi_positive_loss --grouped_batch_sampler --samples_per_image 9 \
    --eeg_backbone_dim ${EEG_BACKBONE_DIM_FIXED} --projector_activation relu \
    --subject_mixup_mode raw_eeg --mixup_type pairwise --subject_mixup_alpha ${MIXUP_ALPHA}"

echo "=============================================================="
echo "Sparse alignment inter-subject | bb=${EEG_BACKBONE_DIM_FIXED} fd=${FEATURE_DIM} relu + mixup"
...
echo "Extracting per-epoch accuracy/sparsity and train active-feature coverage..."
...
echo "Inter-subject avg:   $RUN_DIR/inter_subject_summary.csv"
echo "Per-subject plots:   $PLOT_DIR/*_accuracy_sparsity_epochs.png"
echo "Train active plot:   $PLOT_DIR/train_active_features_at_best.png"
echo "Epoch metrics CSV:   $PLOT_DIR/all_subjects_epoch_metrics.csv"
echo "=============================================================="
