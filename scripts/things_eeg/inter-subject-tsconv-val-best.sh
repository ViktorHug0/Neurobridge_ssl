...
SESSION_DIR="${OUTPUT_DIR_BASE}/tsconv_val_best_${SESSION_TIMESTAMP}"
SESSION_SUMMARY="${SESSION_DIR}/val_best_summary.csv"
mkdir -p "$SESSION_DIR"
...
do
    ...
    do
        CONFIG_NAME="${CONFIG_NAMES[$c_idx]}_featdim${FEATURE_DIM}_seed${SEED}_valbest"
        ...
        do
            ...
            # Pick validation subject as the next one in the list (circularly)
            VAL_ID=$(( (SUB_ID % 10) + 1 ))

            TRAIN_IDS=()
            for TRAIN_SUB_ID in "${ALL_SUBJECT_ARR[@]}"
            do
                if [ "$TRAIN_SUB_ID" -ne "$SUB_ID" ] && [ "$TRAIN_SUB_ID" -ne "$VAL_ID" ]; then
                    TRAIN_IDS+=("$TRAIN_SUB_ID")
                fi
            done

            echo "Subject sub-$(printf "%02d" "$SUB_ID"): Using Val ID $VAL_ID, Train IDs: ${TRAIN_IDS[*]}"
...
echo "TSConv val-best run completed: ${SESSION_SUMMARY}"
