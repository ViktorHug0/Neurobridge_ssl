...
FEATURE_DIM_VALUES=(1024)
SEEDS=(3301)

# Create a dedicated session folder for this entire execution
SESSION_TIMESTAMP=${1:-"20260423-112642"}
SESSION_DIR="${OUTPUT_DIR_BASE}/projector_only_sweep_${SESSION_TIMESTAMP}"
SESSION_SUMMARY="${SESSION_DIR}/sweep_summary.csv"
mkdir -p "$SESSION_DIR"

# Baseline extra args as used in the mixup baseline runs
BASE_EXTRA_ARGS="--multi_positive_loss --grouped_batch_sampler --samples_per_image 9 --eeg_backbone_dim 128"
...
do
    ...
    do
        ...
        if [ "$CONFIG_NAME" == "featdim_16_seed3300" ]; then continue; fi
